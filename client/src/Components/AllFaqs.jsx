...
const AllFaqs = ({ data, title }) => {
  ...
  const filteredFAQ = data.filter(
    (faq) =>
      faq.question.toLowerCase().toString().includes(searchQuery.toLowerCase().toString()) ||
      faq.filteranswer.toLowerCase().toString().includes(searchQuery.toLowerCase().toString())
  );
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:title" content="Faqs - Customer-Dashboard-Pro" />
        <meta property="og:description" content="Faqs Og Description" />
      </Helmet>
      <section className='all-faqs-page-manu'>
        <div className='pages-container container'>
          <div className='all-faqs-page-container'>
            <div className='all-faqs-page-title manu-page-title faq_main_heading'>
              <h1 className='page-title'>Frequently Asked Questions <span>(FAQ)</span></h1>
            </div>
            <div className='page-content all-faqs-page-content'>
              ...
              <div className='faq-items__'>

                <Accordion>
                  {filteredFAQ.map((faq, index) => (

                    <div key={index}>
                      <p style={{ display: 'none' }}><strong>{faq.question}</strong></p>
                      <p>{faq.answer}</p>
                    </div>
                  ))}
                </Accordion>
